...
class XMLParser:
    ...
    def create_empty_products_df(self) -> pd.DataFrame:
        ...
        if type(self.parser_type) == str and type(self.site_acceptor) == str:
            try:
                ...
                empty_product_string = f"{empty_product_slugs[0]}out_{empty_product_slugs[1]}"
                empty_product_feed = self.FEEDS[self.site_acceptor][empty_product_string]
    ...
    def collect_donor_feeds(self) -> list:
        ...
        if type(self.parser_type) == str and type(self.site_acceptor) == str:
            try:
                self.donor_feeds = list(x[self.parser_type] for x in self.FEEDS.values())
                self.donor_feeds.remove(self.FEEDS[self.site_acceptor][self.parser_type])
            except Exception as ex:
                raise ()
