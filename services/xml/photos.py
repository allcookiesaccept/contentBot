...
class PhotoMatcher(XMLParser):
    ...
    def create_empty_products_df(self) -> pd.DataFrame:
        ...
        try:
            xml_offers = self.get_offers_from_xml(self.FEEDS[self.site_acceptor]["without_photos"])
    ...
    def merge_dataframes(
        ...
        df = pd.merge(self.no_photos_dataframe, self.parsed_photos, how="left", left_on="id", right_on="id")

        df.dropna(inplace=True)
        df.drop(columns=["id"], inplace=True)
        df["pictures"] = df["pictures"].apply(
            lambda x: self._replace_image_link(x)
        )

        df.rename(
            columns=dict(zip(df.columns, self.COLUMNS[self.site_acceptor]["photo_upload"])),
            inplace=True,
        )
