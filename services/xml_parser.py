...
class XMLParser:
    ...
    def _replace_description_links(self, description, site_acceptor):
        ...
        new_text = re.sub(r'https://cdn\..*?\.ru/', f'https://cdn\.{site_acceptor}\.ru/', text)
        return new_text
...
class PhotoFiller(XMLParser):
    ...
    def __call__(self, site_acceptor, site_donor):
        ...
        try:
            no_photo = self.get_offers(self.open_xml(self.FEEDS[site_acceptor]['without_photos']))
            no_photo_df = self.extract_empty_products_data(no_photo)
            photo = self.get_offers(self.open_xml(self.FEEDS[site_donor]['with_photos']))
            photo_df = self.extract_photo_products_data(photo)
            merged_df = pd.merge(no_photo_df, photo_df, how='left', left_on='id', right_on='id')
            merged_df.dropna(inplace=True)
            merged_df.drop(columns=['id'], inplace=True)
            merged_df['pictures'] = merged_df['pictures'].apply(lambda x: self._replace_image_link(x, site_acceptor))

            merged_df.rename(columns=dict(zip(merged_df.columns, self.COLUMNS[site_acceptor]['photo_upload'])), inplace=True)
...
class DescriptionFiller(XMLParser):
    ...
    def __call__(self, site_acceptor, site_donor):
        ...
        try:
            no_descriptions = self.get_offers(self.open_xml(self.FEEDS[site_acceptor]['without_description']))
            no_descriptions_df = self.extract_empty_products_data(no_descriptions)
            descriptions = self.get_offers(self.open_xml(self.FEEDS[site_donor]['with_description']))
            descriptions_df = self.extract_products_descriptions(descriptions)
            merged_df = pd.merge(no_descriptions_df, descriptions_df, how='left', left_on='id', right_on='id')
            merged_df.dropna(inplace=True)
            merged_df.drop(columns=['id'], inplace=True)
            merged_df['descriptions'] = merged_df['descriptions'].apply(lambda x: self._replace_description_links(x, site_acceptor))
            file_name = f'descriptions_from_{site_donor}_to_{site_acceptor}_{datetime.date.today()}.csv'
            merged_df.rename(columns=dict(zip(merged_df.columns, self.COLUMNS[site_acceptor]['description_upload'])), inplace=True)
            merged_df.to_csv(file_name, encoding='utf-8', sep=';', index=False)
            return merged_df
        except Exception as e:
            print({e})